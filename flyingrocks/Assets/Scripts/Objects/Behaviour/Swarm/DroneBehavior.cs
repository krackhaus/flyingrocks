...
public class DroneBehavior : MonoBehaviour {
	...
	// the target location of the flock
	...
	protected virtual void FindGoal()
	{
		try
		{
			goalObject = GameObject.Find("FlockGoalPosition");
		}
		catch
		{
			print (goalObject);
			if (!goalObject)
				Debug.LogWarning("No Flock Position Assigned.  Using Boundaries.");
		}
	}
	...
	{
		if (goalObject)
			return new Bounds(goalPos, Vector3.one * 20);
		else
			return new Bounds(swarm.transform.position, new Vector3(swarm.swarmBounds.x, 10000f, swarm.swarmBounds.y));
	}
	...
	protected virtual Vector3 Steer(Vector3 target, bool slowDown)
	{
		...
		Vector3 targetDirection = target - transform.position;// + ((goalObject != null ? goalPos : Vector3.zero) - transform.position);
	...
	protected virtual void OnDrawGizmosSelected()
	{
		...
		Gizmos.DrawLine(goalPos, goalPos - Vector3.down * 10);
	}
}
